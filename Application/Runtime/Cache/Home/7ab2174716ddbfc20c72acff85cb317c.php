		...
		<form action="<?php echo U('/Home/Search/screening');?>" method="post" class="navbar-form navbar-right">
			<div class="form-group">
				...
				<select name="username" id="" class="form-control">
					...
					<?php $__FOR_START_1689167568__=1;$__FOR_END_1689167568__=$maxid;for($i=$__FOR_START_1689167568__;$i < $__FOR_END_1689167568__;$i+=1){ if(is_array($username)): foreach($username as $key=>$n): if(($i == $n['id'])): ?><option value="<?php echo ($v['uid']); ?>"><?php echo ($n["name"]); ?></option><?php endif; endforeach; endif; } ?>
				</select>
				<span></span>
				<span>添加日期：</span>
	      ...
	      <div class="modal-body">
	        <form action="<?php echo U('/Home/Search/screening');?>" method="post" class="navbar-form navbar-right">
				...
				<div class="form-group">
					...
					<select name="username" id="" class="form-control">
						<option value="0">所有</option>
						<?php $__FOR_START_299312666__=1;$__FOR_END_299312666__=$maxid;for($i=$__FOR_START_299312666__;$i < $__FOR_END_299312666__;$i+=1){ if(is_array($username)): foreach($username as $key=>$n): if(($i == $n['id'])): ?><option value="<?php echo ($v['uid']); ?>"><?php echo ($n["name"]); ?></option><?php endif; endforeach; endif; } ?>
					</select>
				</div>
				<div class="form-group">
					<span>添加日期：</span>
	...
	<!-- end -->
      ...
      <div class="panel panel-info">
        ...
        <div class="panel-body">
          <?php if(is_array($searched)): foreach($searched as $key=>$v): if(($v['success'] == 1 )): ?><div class="row well well-sm">
            <?php else: ?>
