		...
		<form action="<?php echo U('/Home/Search/screening');?>" method="post" class="navbar-form navbar-right">
			<div class="form-group">
				...
				<select name="username" id="" class="form-control">
					...
					<?php $__FOR_START_546868844__=1;$__FOR_END_546868844__=$maxid;for($i=$__FOR_START_546868844__;$i < $__FOR_END_546868844__;$i+=1){ if(is_array($username)): foreach($username as $key=>$n): if(($i == $n['id'])): ?><option value="<?php echo ($v['uid']); ?>"><?php echo ($n["name"]); ?></option><?php endif; endforeach; endif; } ?>
				</select>
				<span></span>
				<span>添加日期：</span>
